...
export const portfolios: Portfolio[] = [
  {
    ...
    troubleshooting: [
      {
        trouble: [
          "[문제] 테일윈드 CSS가 적용되지 않는 문제",
          "[문제] 애플 인터렉션 적용 이후 skills와 portfolio 섹션의 호버 이벤트가 위임되어 넓은 범위에서 작동하는 문제",
          "[문제] 메뉴 오퍼시티 애니메이션 효과를 헤더 컴포넌트 단위에서 전체 페이지 컴포넌트로 이동",
          "[문제] 애플 인터렉션 적용 이후 레이아웃이 망가지는 문제",
        ],
        solution: [
          "[해결] 테일윈드에서 임의의 그룹 group-* 으로 부모 요소의 클래스에 따른 선택적 CSS 클래스를 사용하는 중 group-[.showSection_0]:block 클래스가 적용되지 않는 문제를 언더스코어가 아닌 하이픈을 사용하여 해결 했습니다.( 클래스 명의 문제 )",
          "[해결] 애플 인터렉션 구현중 article ( wrapper ) 섹션에 group이 필요하게 되었고 적용 이후에 각 섹션의 이벤트가 위임되어 버렸고 이런 문제를 해결하기 위해 group을 섹션별로 나누어 묶었습니다. ex ) group/portfolio , group/skills",
          "[해결] 기존 메뉴 오퍼시티 애니메이션만 구현하고 애플 인터렉션을 적용하기 위해 헤더 컴포넌트에서 페이지 컴포넌트로 애니메이션을 이용하여 스크롤 이벤트 한개로 통합 하였습니다.",
          "[해결] 각 섹션의 높이를 자바스크립트로 컨트롤하여 컴포넌트 마운트, 리사이즈 이벤트에 걸었으나 좌우 폭을 줄이는 부분에서 컴포넌트 높이가 px로 설정되어있어서 영역이 제대로 잡히지 않았습니다. 0번, 5번섹션만 애플 인터렉션을 사용하므로 해당 섹션 이외의 섹션에는 고정 높이를 사용하지 않도록 정정하였습니다.",
        ],
      },
    ],
    retrospect:
