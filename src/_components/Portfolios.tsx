...
export const portfolios: Portfolio[] = [
  {
    ...
    troubleshooting: [
      {
        trouble: [" 테일윈드 CSS가 적용되지 않는 문제"],
        solution: [
          " 테일윈드에서 임의의 그룹 group-* 으로 부모 요소의 클래스에 따른 선택적 CSS 클래스를 사용하는 중 group-[.showSection_0]:block 클래스가 적용되지 않는 문제를 언더스코어가 아닌 하이픈을 사용하여 해결 했습니다.( 클래스 명의 문제 )",
        ],
      },
      {
        trouble: [
          " 애플 인터렉션 적용 이후 skills와 portfolio 섹션의 호버 이벤트가 위임되어 넓은 범위에서 작동하는 문제",
        ],
        solution: [
          " 애플 인터렉션 구현중 article ( wrapper ) 섹션에 group이 필요하게 되었고 적용 이후에 각 섹션의 이벤트가 위임되어 버렸고 이런 문제를 해결하기 위해 group을 섹션별로 나누어 묶었습니다. ex ) group/portfolio , group/skills",
        ],
      },
      {
        trouble: [
          " 메뉴 오퍼시티 애니메이션 효과를 헤더 컴포넌트 단위에서 전체 페이지 컴포넌트로 이동",
        ],
        solution: [
          " 기존 메뉴 오퍼시티 애니메이션만 구현하고 애플 인터렉션을 적용하기 위해 헤더 컴포넌트에서 페이지 컴포넌트로 애니메이션을 이용하여 스크롤 이벤트 한개로 통합 하였습니다.",
        ],
      },
      {
        trouble: ["애플 인터렉션 적용 이후 레이아웃이 망가지는 문제"],
        solution: [
          " 각 섹션의 높이를 자바스크립트로 컨트롤하여 컴포넌트 마운트, 리사이즈 이벤트에 걸었으나 좌우 폭을 줄이는 부분에서 컴포넌트 높이가 px로 설정되어있어서 영역이 제대로 잡히지 않았습니다. 0번, 5번섹션만 애플 인터렉션을 사용하므로 해당 섹션 이외의 섹션에는 고정 높이를 사용하지 않도록 정정하였습니다.",
        ],
      },
      {
        trouble: [
          "스크롤을 굉장히 빠르게 이동했을 때 랜딩과 테이크오프 섹션의 텍스트가 사라지지 않는 문제",
        ],
        solution: [
          "스크롤 이벤트의 경우 굉장히 빠르게 움직였을 때 건너뛰는 이벤트의 숫자가 많아지게 되는 문제가 있습니다. 이로 인해 텍스트에 opacity가 제대로 0 이 되지 않는 문제가 생겼는데 0번 섹션과 5번 섹션이외의 섹션에서 해당 텍스트의 opacity를 0으로 처리해줌으로 버그를 해결했습니다.",
        ],
      },
      {
        trouble: [
          "useRouter, usePathname 등으로 현재 페이지를 판별하지 못하는 문제",
        ],
        solution: [
          "페이지 내 id로 섹션을 이동하게되면 URL에 hash가 남게되는데 이것을 useRouter, usePathname 등으로 판별할 수 없습니다. window.location.hash 을 사용하여 할 순 있으나 약간의 트릭을 썼습니다. hash로 페이지 내 섹션을 이동하게 되면 margin을 제외한 실제 콘텐츠의 영역부터 보여지게 됩니다. 제가 만든 스크롤 이벤트는 margin을 포함하여 섹션을 구분하기 때문에 모든 섹션의 margin top 을 설정하여 네비게이션에 활용하는 방식으로 해결했습니다.",
        ],
      },
    ],
    retrospect:
  ...
  },
  {
    ...
    troubleshooting: [
      {
        trouble: [
          " 인증 메일 발송 오류: 회원가입 중 첫 번째 인증 메일 발송은 성공했지만, 이후 다른 이메일로 가입을 시도할 때 인증 메일 발송에 실패했다는 오류가 반복적으로 발생했습니다.",
          " 데이터 로딩 지연: 로그인 성공 후, 데이터베이스에서 데이터를 로딩하는데 시간이 과도하게 오래 걸리는 문제가 있었습니다.",
        ],
        solution: [
          ` Supabase의 무료 플랜에서는 1시간에 3회의 메일 발송 제한이 있다는 것을 공식 문서를 통해 확인했습니다.
...
도메인을 구입하고 Resend와 연동한 후, 이를 다시 Supabase와 연결하여 메일 발송 문제를 해결했습니다.`,
          ` 이 문제는 Supabase의 무료 버전에서 발생하는 제약으로, 현재는 이를 해결할 방법이 없었습니다.
개발 단계에서는 비용 부담 때문에 그대로 두었지만, 이후 더 나은 서비스를 배포할 때 유료 버전으로 업그레이드하는 방안을 고려하고 있습니다.`,
  ...
  },
  {
    ...
    troubleshooting: [
      {
        trouble: [
          "라디오와 체크박스 타입의 설문 작성 시 '기타' 항목은 반드시 단 하나만 존재해야 하며, 항상 옵션 목록의 마지막에 위치해야 한다는 전제 조건을 가지고 있었으나, '기타 항목 추가' 버튼을 클릭할 때마다 기타 항목이 중간에 섞여 들어가는 문제가 발생했습니다.",
        ],
        solution: [
          " 옵션 객체에 other이라는 플래그를 추가하여 '기타' 항목의 존재 여부를 확인하도록 했습니다. 이 플래그가 true인 경우, 새로운 옵션은 항상 마지막 인덱스 앞에 추가되도록 처리하였으며, false인 경우에는 새로운 옵션이 항상 배열의 끝에 추가되도록 하여 '기타' 항목이 가장 마지막에 위치하도록 수정했습니다.",
        ],
      },
    ],
    viewsImage: [
  ...
  },
  {
    ...
    troubleshooting: [
      {
        trouble: [" 최초 웹 접근시 MSW import가 제대로 작동하지 않는 이슈"],
        solution: [
          ` MSW 라이브러리를 Next.js 14 App Router에서 사용하는 과정에서, 브라우저의 서비스 워커가 제대로 작동하지 않는 문제가 있었습니다. 여러 레퍼런스를 참고했지만 해결할 수 없었고, 스터디 리더와 함께 콘솔 출력을 확인하며 작동 시점과 작동하지 않는 시점을 분석했습니다. 결국, 서비스 워커를 올바르게 작동시키기 위해 브라우저가 준비되는 즉시 비동기로 임포트하는 방식을 채택했고, 사용자 경험을 저해하지 않기 위해 로딩 스피너를 적용해 문제를 해결했습니다.
  ...
  },
  {
    ...
    troubleshooting: [
      {
        trouble: [" 북마크 페이지에서 북마크 해제시 화면 반영이 되지않던 문제"],
        solution: [
          " 북마크 해제시 로컬스토리지 데이터가 변경되어도 현재 화면에서 관리하는 로컬스토리지 데이터 상태가 최초 마운트시에만 셋팅되어 로컬스토리지가 변경되어도 화면 리렌더링이 발생하지 않고 모달창 또한 종료되지 않았습니다. useHeartToggle Hook에 하트버튼을 해제하는 상황에 작동할 콜백함수를 제공하고 해제시에 관리하던 로컬스토리지 상태를 변경하고 모달을 종료하는 로직을 추가하여 해결하였습니다.",
        ],
      },
    ],
    retrospect:
